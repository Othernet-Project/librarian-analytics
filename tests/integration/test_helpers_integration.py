...
def test_get_stats(populated_database):
    ...
    stats = mod.get_stats(databases.analytics)
...
def test_get_stats_bitstream(populated_database):
    ...
    expected_stream = b''.join(d.payload for d in sorted_data)
    ids, bitstream = mod.get_stats_bitstream(databases.analytics)
...
def test_prepare_device_id(random_path, helpers):
    ...
    with mock.patch(MOD + '.generate_device_id') as gdid:
...
def test_get_payload(random_path, populated_database):
    ...
    sorted_data = sorted(test_data, key=lambda x: x.timestamp)
    expected_stream = b''.join(d.payload for d in sorted_data)
    ids, payload = mod.get_payload(databases.analytics, random_path)
    assert len(list(ids)) == len(test_data)
