...
@mock.patch(MOD + '.get_stats')
def test_get_stats_bitstream(get_stats):
    get_stats.return_value = [{'id': 1, 'payload': 'foo'},
                              {'id': 2, 'payload': 'bar'}]
    ids, payload = mod.get_stats_bitstream(db=mock.Mock())
    assert list(ids) == [1, 2]
...
def test_get_stats_bitstream_no_stats(get_stats):
    ...
    assert mod.get_stats_bitstream(db=mock.Mock()) == ([], b'')
